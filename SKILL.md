...
**重要な罠 1 (TBD)**: `bool("TBD") == True` のため、`if is_implemented(...):` の素朴な分岐では
TBD が True 扱いに潰れる。**厳密判定は `is_implemented(...) is True` または
`requires_tbd_warning()` を使う**。

**重要な罠 2 (カテゴリ / 2026-06-05 追加)**: **メガストーンは `is_implemented("megastones", id)` で照会する**。
`is_implemented("items", "greninjite")` のように `items` カテゴリで照会すると、megastones に登録された
エントリが見つからず **デフォルト True に落ちる (Default-Permissive Trap)** → TBD/False を見逃す。
カテゴリ対応: 持ち物=`items` / 技=`moves` / ポケ=`pokemon` / **メガストーン=`megastones`**。
例: ゲッコウガナイトの実装可否は必ず `is_implemented("megastones", "greninjite")`。
(API 実装根拠: `lib/champions_overlay.py` の `is_implemented` は `entry is None` 時に `return True` = Default-Permissive。2026-06-05 実機検証済)

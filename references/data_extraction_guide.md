...
## 取得後の解釈ステップ: 対策視点での読み方

> **目的**: DB の数字を「実採用テンプレ」に直結させない。
> 詳細用語定義は `SKILL.md` 第 14 節 (DB の正しい読み方) を参照。

### Step 1: 全体使用率 TOP30 → 「対策地図」として読む

- 高使用率ポケ = 「環境が対策を積んでいる対象」
- 自パに「TOP10 各ポケへの回答役」があるかを 1 列マッピング
- 回答役欠落ポケが「自パの穴」

### Step 2: 個別ポケの技/持ち物 TOP → 「使い方学習」として読む

- 技 TOP6 = そのポケの **立ち回り定石**
- 持ち物 TOP3 = そのポケの **定石対応**
- **このデータで実採用を決めない**

### Step 3: 実採用 → 「自パーティ補完」で決定

- Step 2 で得た定石を参考にしつつ
- 自パ 5 体で既に埋まっている役割は除外
- 残り枠で「自パに足りない役割」を埋める技/持ち物を選ぶ

### Step 4: 2 DB 比較 → 「事実確認の場」として読む

- `champs.pokedb` と `pokechamdb` の数字一致は **当然** (同じ統計系)
- 一致は信頼度の根拠ではなく、**パース誤差・取得ミス検出** の手段
- 真の信頼度は note 構築記事 / 強プレイヤー動画 (24-72h) で別途検証

### 解釈アンチパターン

| NG | OK |
|---|---|
| 採用率 1 位の技 4 つをコピー | 自パで埋まってない役割の技を選ぶ |
| 2 DB 一致 → 信頼性高い | 2 DB 一致 = パース誤差なし、信頼性は別検証 |
| TOP5 ポケ集めれば最強構築 | TOP5 = 最も対策される側、独自視点の補完が必須 |

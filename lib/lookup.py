...
def get_move(mid: str) -> Optional[dict]:
    """Return raw Showdown move dict (no Champions overlay applied).

    For Champions-aware lookups (UI display, build advice), use
    `lib.champions_overlay.get_move_overlayed` instead.
    For damage calc input, use `get_move_raw` (alias of this function).
    """
    return get_moves().get(mid)


def get_ability(aid: str) -> Optional[dict]:
    """Return raw Showdown ability dict. See get_move docstring for overlay note."""
    return get_abilities().get(aid)


def get_item(iid: str) -> Optional[dict]:
    """Return raw Showdown item dict. See get_move docstring for overlay note."""
    return get_items().get(iid)


# -----------------------------------------------------------------------------
# Explicit "raw" aliases (lru_cache pollution prevention).
#
# These return the same Showdown raw values as get_move/get_item/get_ability
# but with a name that documents intent: damage calc and other consumers
# requiring untouched Showdown semantics should call *_raw to make that
# requirement explicit. lib.champions_overlay.get_*_overlayed wraps these.
# -----------------------------------------------------------------------------


def get_move_raw(mid: str) -> Optional[dict]:
    return get_moves().get(mid)


def get_item_raw(iid: str) -> Optional[dict]:
    return get_items().get(iid)


def get_ability_raw(aid: str) -> Optional[dict]:
    return get_abilities().get(aid)
